...
const POSTCODE_MAP: Record<string, { city: string; state: string }> = {
...
function lookupPostcode(postcode: string): { city: string; state: string; valid: boolean } | null {
  const entry = POSTCODE_MAP[postcode];
  if (entry) return { ...entry, valid: true };
  ...
  if (state) return { city: '', state, valid: true };
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { makeCode, modelCode, makeYear, region, postcode } = req.query as {
        makeCode?: string;
        modelCode?: string;
        makeYear?: string;
        region?: Region;
